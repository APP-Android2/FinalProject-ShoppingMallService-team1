...
class MyReviewFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // MyReviewFragment 가 실행될 때 하단바가 보이지 않도록
        navigationActivity.activityNavigationBinding.bottomNavigationView.isVisible = false

        // 장바구니에서 다양한 운동 시설 보러가기 버튼 클릭 후 NavigationActivty(에서 MyReviewFragment)로 돌아왔을 때 실행
    ...
    override fun onDestroy() {
        super.onDestroy()
        // MyReviewFragment 가 제거될 때 하단바가 보이도록
        ...
        fragmentMyReviewBinding.apply {
            toolbarMyReview.apply {
                ...
                setOnMenuItemClickListener {
                    when(it.itemId){
                        R.id.menuMyReviewShopping -> {
                            val myReviewIntent = Intent(navigationActivity, ShoppingCartActivity::class.java)
                            shoppingCartActivityLauncher.launch(myReviewIntent)
                        }
                    }
